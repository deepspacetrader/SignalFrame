...
export function PredictionsPanel({ onAIRequired }: { onAIRequired: () => void }) {
    ...
    `;
        ...
        } else if (aiConfig.provider === 'nim') {
          const systemPrompt = 'You must respond with valid JSON only. Do not include markdown formatting, explanations, or text outside the JSON object.';
          response = await NimService.generate(aiConfig.model, prompt, systemPrompt, 'json', { max_tokens: 25000 });
        } else if (aiConfig.provider === 'llamacpp') {
          const systemPrompt = 'You must respond with valid JSON only. Do not include markdown formatting, explanations, or text outside the JSON object.';
          response = await LlamaCppService.generate(aiConfig.model, prompt, systemPrompt, 'json', { max_tokens: 25000 });
        } else {
            ...
            <div className="p-4 border-t border-white/5 bg-black/20">
                <div className="flex flex-col md:flex-row gap-4">
                    ...
                    <button
                        ...
                        className="px-6 py-3 bg-accent-primary/20 hover:bg-accent-primary/40 border border-accent-primary/30 text-accent-primary rounded-lg transition-colors disabled:opacity-50 uppercase text-xs font-bold tracking-wider whitespace-nowrap flex items-center justify-center gap-2"
                    >
                        {isLoading && (
                        ...
                        {isLoading ? 'Projecting...' : 'Generate Trajectory'}
                    </button>
                </div>
            </div>
