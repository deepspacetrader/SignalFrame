...
export function RSSSettings({ onAIRequired }: { onAIRequired?: () => void }) {
    ...
    return (
        ...
        >
            <div className="bg-bg-card border border-white/10 p-8 max-w-6xl w-full max-h-[90vh] overflow-y-auto relative">
                ...
                <div className="space-y-8">
                    ...
                    <div>
                        ...
                        {(() => {
                            // Group feeds by category and sort alphabetically within each category
                            const groupedFeeds = KNOWN_RSS_FEEDS.reduce((acc, feed) => {
                                if (!acc[feed.category]) {
                                    acc[feed.category] = [];
                                }
                                acc[feed.category].push(feed);
                                return acc;
                            }, {} as Record<string, typeof KNOWN_RSS_FEEDS>);

                            // Sort feeds alphabetically within each category
                            Object.keys(groupedFeeds).forEach(category => {
                                groupedFeeds[category].sort((a, b) => a.source.localeCompare(b.source));
                            });

                            // Sort categories alphabetically
                            const sortedCategories = Object.keys(groupedFeeds).sort();

                            return sortedCategories.map(category => (
                                <div key={category} className="bg-bg-darker/30 border border-white/10 p-4 mb-3">
                                    <h5 className="text-md font-semibold text-text-primary mb-3 flex items-center gap-2">
                                        {category}
                                    </h5>
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                                        {groupedFeeds[category].map((feed) => {
                                            const configFeed = rssConfig.defaultFeeds.find(f => f.url === feed.url);
                                            const isEnabled = configFeed?.enabled ?? true;
                                            return (
                                                <div
                                                    key={feed.url}
                                                    className={`bg-white/5 border p-3 transition-all hover:bg-white/10 ${isEnabled
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        {feed.logo ? (
                                                            <>
                                                                ...
                                                                />
                                                                <div className="w-6 h-6 bg-accent-primary/20 rounded items-center justify-center text-xs text-accent-primary font-bold" style={{ display: 'none' }}>
                                                                    {feed.source.charAt(0)}
                                                                </div>
                                                            </>
                                                        ) : (
                                                            <div className="w-6 h-6 bg-accent-primary/20 rounded flex items-center justify-center text-xs text-accent-primary font-bold">
                                                                {feed.source.charAt(0)}
                                                            </div>
                                                        )}
                                                        ...
                                                        <button
                                                            ...
                                                            className={`relative w-10 h-5 rounded-full transition-all duration-300 ${isEnabled
                                                                ? 'bg-accent-primary'
                                                                : 'bg-white/10 border border-white/20'
                                                                }`}
                                                        >
                                                            <div className={`absolute top-0.5 w-3.5 h-3.5 rounded-full bg-white shadow-lg transition-all duration-300 ${isEnabled ? 'left-6' : 'left-0.5'
                                                                }`}></div>
                                                        </button>
                                                    </div>
                                                </div>
                    ...
                    <div className="bg-bg-darker/30 border border-white/10 p-6">
                        <h4 className="text-lg font-semibold text-text-primary mb-4 flex items-center gap-2">
                        ...
                        <div className="bg-black/30 border border-white/10 p-4 mb-4">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                                <input
                                    ...
                                    className="bg-white/5 border border-white/10 p-3 text-text-primary focus:border-accent-primary outline-none transition-all"
                                />
                                <input
                                    type="url"
